...
def generate_proxies():
    ...
    try:
        ...
        vast_print("Unable to scrape proxies, please use a VPN and reopen Vast.")
        input()
        sys.exit()
...
def group_scraper():
    ...
    clear()
    print_text()
    open("data/groups/groups.txt", "w+").write("")
            ...
            if isinstance(channel, discord.GroupChannel):
                if int(channel.owner.id) == user_id:
                    try:
                        ...
                        open("data/groups/groups.txt","a+").write(f"{channel.id}\n")
                    except Exception as e: print(e)
...
def get_groups():
    ...
    clear()
    print_text()
    vast_print(f"You have {count} groups")
...
def check(token):
    ...
    try:
        r = requests.get(f'https://discord.com/api/v9/users/@me', headers=headers, proxies={"http": f"{get_random_proxy()}"})
        if r.status_code == 200:
            return r.json()['id']
...
def check_token(token):
    ...
    try:
        r = requests.get(f'https://discord.com/api/v9/users/@me/guild-events', headers=h, proxies={"http": f"{get_random_proxy()}"})
        tk = token
        try:
            tk = token[:25] + "*" * 34
        except:
            tk = "*" * len(token)
        if r.status_code == 200:
            token_work(f'{tk} is valid!')
            open("workingtokens.txt", "a+").write(f"{token}\n")

        else:
            text = r.text
            if "You need to verify your account" in text:
                token_nowork(f'{tk} is unverified/locked')
            elif "Unauthorized" in text:
                token_nowork(f'{tk} is invalid')
            else:
                token_nowork('Failed to check!')
    except requests.exceptions.ProxyError:
        vast_print("DEAD PROXY")
        check_token(token)
        ...
        try:
            ...
            if worked:
                def send(group_id):
                    ...
                    try:
                        messagedata = {"content":advert ,"nonce":"","tts":"true"}
...
def groupadder(token, group_id, user_id, user_choice):
    try:
        ...
        if user_choice == "add":
                req = requests.put(f"https://ptb.discord.com/api/v8/channels/{group_id}/recipients/{user_id}", headers=headers, proxies={"http": f"{get_random_proxy()}"})
                print(req.json())
        ...
        if user_choice == "remove":
            requests.delete(f"https://discord.com/api/v8/channels/{group_id}/recipients/{user_id}", headers=headers)
...
def menu_choice(choice : int):
    ...
    if choice == 1: # GROUP CREATOR
        ...
        vast_print(f"Loaded {len(tokens)} tokens")
        generate_proxies()
        open("workingtokens.txt","w+").write("")
        for token in tokens:
...
def auth():
    ...
    if not os.path.exists("data/config/license.json"):
        ...
        if Authentication.get_uuid().strip() == "03000200-0400-0500-0006-000700080009":
            check_license = Authentication.get_user('license', license)
        else:
            check_license = Authentication.get_user('license')
